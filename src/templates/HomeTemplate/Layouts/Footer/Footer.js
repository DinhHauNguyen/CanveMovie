...
const android = require('../../Layouts/android-logo.png')
const apple = require('../../Layouts/apple-logo.png')
const facebook = require('../../Layouts/facebook-logo.png')
const zalo = require('../../Layouts/zalo-logo.png')

export default function Footer() {
    const { arrayTheaterSystem } = useSelector(state => state.ManageTheaterReducer)
    const arrSystem = _.map(arrayTheaterSystem, (sysTheater) => _.pick(sysTheater, ['logo']))
    return (
        <footer className="py-6 text-white bg-stone-700">
            <div className="container px-6 mx-auto space-y-6 divide-y divide-coolGray-400 md:space-y-12 divide-opacity-50">
                <div className="grid grid-cols-4">
                    ...
                    <div className="">
                        <p className="pb-1 text-lg text-start font-light">PARTNER</p>
                        <div className='grid grid-cols-2'>
                            {arrSystem.map((item, index) => {
                                return <div key={index}>
                                    <img src={item.logo} style={{ width: 50 }}></img>
                                </div>
                            })}
                        </div>
                    </div>
                    <div style={{marginLeft: '150px'}}>
                        <p className="pb-1 text-lg text-left font-light">MOBILE APP</p>
                        <div style={{ display: 'flex' }}>
                            <img src={android} style={{ width: 40 }} />
                            <img src={apple} style={{ width: 40, marginLeft: 15 }} />
                        </div>
                    </div>
                    <div style={{marginLeft: '150px'}}>
                        <p className="pb-1 text-lg text-left font-light">SOCIAL</p>
                        <div style={{ display: 'flex' }}>
                            <img src={facebook} style={{ width: 40 }} />
                            <img src={zalo} style={{ width: 40, marginLeft: 15 }} />
                        </div>
                    </div>
                </div>
                <div className="grid justify-center pt-6 lg:justify-between">
                    ...
                    <div className=" flex justify-center pt-4 space-x-4 lg:pt-0 lg:col-end-13">
                        ...
                        <a rel="noopener noreferrer" href="#" title="Twitter" className="flex items-center justify-center w-10 h-10 rounded-full dark:bg-violet-400 dark:text-coolGray-900">
                            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 50 50" fill="white" className="w-5 h-5">
